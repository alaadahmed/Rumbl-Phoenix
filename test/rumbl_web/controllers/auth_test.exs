defmodule RumblWeb.AuthTest do
  ...
  test "login with a valid username and pass", %{conn: conn} do
    user = user_fixture(username: "memo", email: "memo@test", password: "secret")
    {:ok, conn} = Auth.login_by_email_and_pass(conn, "memo@test", "secret")
    assert conn.assigns.current_user.id == user.id
  end

  test "login with a not found user", %{conn: conn} do
    assert {:error, :not_found, _conn} = Auth.login_by_email_and_pass(conn, "memo@test", "secret")
  end
